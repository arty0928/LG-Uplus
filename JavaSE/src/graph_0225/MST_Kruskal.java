 ...
 *     n=100일때 최대 간선 n(n-1)/2    5000C99    5000!/4901!99! ==> (완전 탐색 X)
 *  - 그리디를 응용한 알고리즘   
 *  	1. 원하는 기준으로 정렬 (크루스칼)
 *  	2. 정렬할 수 없는 상황이면, 현 시점에서 할 수 있는 것을 탐색해서 최고 찾기 (프림)
 */

/*
 * 간적크, 간많프 (간선이 적으면 크루스칼, 간선이 많으면 프림)
 */
 ...
 * 2. Edge정렬 	=> 시간 복잡도   O(ElogE) -> 엣지를 가지고 정렬
...
//	    		비용 누적
        		result += edge.weight;
//	    		연결수 증가 및 V-1개인지 확인 ( V-1개이면 연결 끝남)
        		if(++count == V-1) break;
        	}
		}
