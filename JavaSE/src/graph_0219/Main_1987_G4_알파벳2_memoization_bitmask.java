 ...
 *   2. 방문 배열을 [R][C]만큼 할 필요 없이 [26]개의 일차원 배열 대신 bitmask 이용
 *    
 *    	비트마스크를 안 쓰면 이차원 배열에서 해당 위치에서 가본 문자열을 파악하려면 3차원 배열이됨...
 *    
 ...
 *  164ms
 */
public class Main_1987_G4_알파벳2_memoization_bitmask {
    ...
    public static void main(String[] args) throws Exception {
        ...
        dfs(0, 0, 1 << map[0][0], 1); //1 << map[0][0] 방문처리
    ...
    private static void dfs(int r, int c, int bit, int cnt) {
        ...
        for (int i = 0; i < 4; i++){
            ...
            if ((bit & 1 << map[nr][nc]) == 0 &&
            		...
            		// map[nr][nc] 위치에 알파벳을 bit에 추가한 후 이전 기록과 같은지 체크
            		// 0이면, bit에 해당 알파벳 추가한 것이 이미 있는지 == 안 가본 거라면
                    (bit | 1 << map[nr][nc]) != visited[nr][nc]) { // 같으면 안가고 ==> 탐색 해봤자 같은 경우의 수  
            	
            	// 이전 기록과 같지 않은 알파벳 조합이라면 dfs로 탐색
