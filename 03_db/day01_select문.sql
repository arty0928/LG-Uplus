...
-- ==? where 절을 통한 조건을 저장하지 않으면 모든 행을 조회한다
select * from members;
...
-- from emp
-- where employeeNO>7500;  --employeeNo는 alias여서 where절에서 사용하면 error 발생

select empno as employeeNo, ename as employeeName, sal as salary
from emp
where empno > 7500
order by employeeNo;       -- order by 절에서는 컬럼 alias를 사용할 수 있다.